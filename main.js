...
;// CONCATENATED MODULE: ./src/js/untrello/localstorage.js
class LocalStorage {
  static data = [{
    id: 1,
    name: "TODO",
    tasks: ["Lorem ipsum dolor sit amet consectetur adipisicing elit.", "Repudiandae ad earum reprehenderit dolorum cumque necessitatibus.", "Iusto aliquid excepturi voluptates eveniet magni ipsa vitae distinctio repellat quaerat enim nostrum, nam fugiat."]
  }, {
    id: 2,
    name: "IN PROGRESS",
    tasks: ["Lorem ipsum dolor sit amet consectetur adipisicing elit.", "Repudiandae ad earum reprehenderit dolorum cumque necessitatibus.", "Iusto aliquid excepturi voluptates eveniet magni ipsa vitae distinctio repellat quaerat enim nostrum, nam fugiat."]
...
;// CONCATENATED MODULE: ./src/js/untrello/untrello.js
...
class UnTrello {
  ...
  mouseUp = e => {
    ...
    this.data[thisColumnId].tasks.splice(previousId, 0, this.actualItem.innerText);

    // удаляем старый элемент из объекта data
    this.data[this.removeDataElement.columnId].tasks.splice(this.removeDataElement.itemId, 1);
  ...
  addForm() {
    ...
    formOk.forEach(el => {
      el.addEventListener("click", e => {
        ...
        const thisInput = el.closest(".add-task-form").querySelector(".add-task-input");
        if (thisInput.value !== "") {
